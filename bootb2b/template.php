 ...
 * @param $form_state
 */
function bootb2b_form_views_form_ft_commerce_cart_block_default_alter(&$form, &$form_state) {
  if (!empty($form["edit_delete"])) {
    foreach ($form["edit_delete"] as &$delete_button) {
      if (is_array($delete_button) && isset($delete_button["#value"])) {
        $delete_button["#value"] = t("Remove");
        $delete_button["#ajax"] = [
          "callback" => "_bootb2b_delete_from_cart_form_ajax_handler",
          "event" => "click",
        ];
      }
    }
 ...
 * @throws \Exception
 */
function _bootb2b_delete_from_cart_form_ajax_handler($form, $form_state) {
  $commands = [];

  $form_id = $form["#id"];
  if (!empty($form_state["triggering_element"]["#name"])) {
    $commands[] = ajax_command_remove("#$form_id .views-row:has(button[name='{$form_state["triggering_element"]["#name"]}'])");
  }
  ...
  if (!empty($summary["count"]) && $summary["count"] > 0) {
    $formatted_count = format_plural($summary["count"], "item", "items");
    $formatted_amount = commerce_currency_format($summary["total"], $summary["currency"]);

    $output = "";
    $output .= "<div class=\"line-item-summary\">";
    $output .= "<div class=\"line-item-quantity\">";
    $output .= "<span class=\"line-item-quantity-raw\">" . $summary["count"] . "</span> ";
    $output .= "<span class=\"line-item-quantity-label\">" . $formatted_count . "</span>";
    $output .= "</div>";
    $output .= "<div class=\"line-item-total\" >";
    $output .= "<span class=\"line-item-total-label\">" . t("Total:") . "</span> ";
    $output .= "<span class=\"line-item-total-raw\">" . $formatted_amount . "</span>";
    $output .= "</div>";
    $output .= "</div>";

    $commands[] = ajax_command_replace("#$form_id .line-item-summary", $output);
  }
  else {
 ...
 * @param $variables
 */
function bootb2b_preprocess_html(&$variables) {
  _bootb2b_remove_line_item_modal($variables);
}

/**
 * @param $variables
 */
function _bootb2b_remove_line_item_modal(&$variables) {
  $variables["page"]["page_bottom"]["bergin"] = [
    "#markup" => "
      <div class=\"modal fade\" id=\"removeLineItemDialog\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"removeLineItemDialog\" aria-hidden=\"TRUE\">
        <div class=\"modal-dialog\">
          <div class=\"modal-content\">
            <div class=\"modal-header\">
              <h4 class=\"modal-title\" id=\"removeLineItemDialogLabel\">Do you really want to remove this item from your cart?</h4>
            </div>
            <div class=\"modal-body\">
              <button type=\"button\" class=\"btn btn-warning btn-cancel\" data-dismiss=\"modal\">Cancel</button>
              <button type=\"button\" class=\"btn btn-danger btn-remove\" data-dismiss=\"modal\">Yes, remove it</button>
            </div>
          </div>
        </div>
      </div>
